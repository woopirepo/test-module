...
def run_bert_score_evaluation(df, answer_1_column_name,answer_2_column_name,similarity_threshold):
  
    
    eval_metrics = EvaluationMetrics(similarity_threshold=similarity_threshold)
    
    precisions, recalls, f1_scores, similarity_values, is_similar = eval_metrics.evaluate(df[answer_1_column_name], df[answer_2_column_name])
    ...
    csv_file_path = f"{precisions}, " + "_ - - -evaluated.csv"
